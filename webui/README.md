...
- 顶部导航默认进入 `Analyze`，可见顺序固定为 `Analyze`、`Memory`、`System Context`、`Tools`、`Settings`；Metadata 不再是顶层 tab，作为 System Context 页面内的 `Metadata` 标签页提供。
- 本地开发地址（`127.0.0.1`、`localhost` 或 `::1`）首次打开且浏览器没有保存 API Key 时，WebUI 会自动使用本地脚本默认的 `dev-token`；非本地地址不会自动填充密钥。
...
- `System Context`：已切换为 V2 System Context + V2 Metadata 工作台，页面内通过 `System Context` / `Metadata` 标签页切换，避免两个大型模块纵向铺开；直接调用 Python V2 `/api/v2/skills*`、`/api/v2/system-context/*`、`/api/v2/metadata/instances` 和 `/api/v2/metadata/*`：支持 V2 Skill 列表/详情、Markdown Skill import、显式 Skill selection preview、`skills.zip` 下载、V1-compatible System Context resource 列表/创建/编辑、版本追加/编辑/激活、资源+Metadata adapter prompt preview、JSON/YAML/CSV/openGemini 内容或 URL 的导入预览、确认、直接导入、导入历史、实例列表、用已保存 raw JSON 刷新实例、实例删除和 snapshot JSON 查看。旧 Rust Skills/Metadata 页面不再默认渲染。
- `Tools / Tool plugins`：已切换为 V2 Tools 工作台，调用 Python V2 `/api/v2/tools` 展示工具目录和 `sourceBuiltAnalyzers` 状态面板；工具目录优先读取 `tools`，并兼容读取同内容的 `toolPlugins` alias，避免前后端版本短暂不一致时列表为空；当认证缺失、认证失败或 catalog 尚未加载时，空态展示当前加载/认证状态而不是误报 `No V2 tools`。工具目录展示 `source`、`tags`、`readOnly`、`editable`、`exportable`、`manualOnly`、文件数量约束、`acceptedSuffixes`、`outputViews`、`match`、`paramsTemplate` 和 `paramsSchema`，source-built analyzer 面板展示四个 submodule analyzer 的 registered/enabled/runnable、command path、命令存在性 `exists/no file`、可执行性 `exec/no exec`、timeout、max input files 和 unavailable reason，支持下载 `/api/v2/exports/tools.zip`。页面同时支持两种 V2 原生运行方式：输入 V2 `run_id` 后按选中工具的 `paramsTemplate` 预填 params，通过 `/api/v2/mcp/task/:run_id` 调用 task MCP，其中 configured command/source-built analyzer 路由到 `logagent.run_domain_tool`，metadata built-ins 直接调用同名 MCP tool，Fetch 调用 `logagent.fetch`，manual-only 工具提示改用下方 Manual `tool_run`；或在 Manual `tool_run` 区复用/自动创建 Workspace，上传匹配文件或用 `params.inputFiles` 复用 Workspace 已知输入，再调用 `/api/v2/tools/:tool_id/runs`；当 `params.inputFiles` 非空时必须填写已有 Workspace id 且不能同时选择上传文件，避免创建空 Workspace 后引用不存在的逻辑路径。文件选择器会把 `.ext` / `*.ext` 转换成浏览器可识别的 accept 扩展并忽略非扩展 pattern，随后刷新 `/api/v2/tools/runs`、轮询选中的非终态 run、读取 `/api/v2/tools/runs/:run_id/result` 和 `/api/v2/tools/runs/:run_id/artifacts`，并通过带 Authorization header 的 `/api/v2/artifacts/:artifact_id` 下载 upload、evidence 和 support artifacts；metadata built-ins 成功结果会展示实例列表、snapshot 摘要或 field/tag field 表；`logagent.preprocess_log_package` 成功结果会专门展示 upload/file/node/tool input 计数、manifest/grep/tool_inputs 路径、log group 汇总、节点表和 tool input index 表；configured command/source-built analyzer 成功结果会展示 status、exit code、timed out、input file、result/stdout/stderr 路径、findings 表、stdout/stderr preview 和 parsed stdout；多输入手动 configured run 的聚合结果会展示 aggregate status、inputFiles、per-input results、artifactPaths 和 raw JSON；`logagent.huawei_cloud_package_sync` 成功结果会展示 OBS PUT/HEAD、GaussDB update/query、对象地址、SQL/timing、query rows、warning/error 和凭据环境变量名；`pprof_analyzer` 成功结果会专门展示 profile type、sample、total、duration、top 函数表和 result/top/tree/raw/stderr/SVG artifact 路径，其他工具继续展示 JSON result。旧 Rust 独立上传型 Tool Runner 页面不再默认渲染。
...
- Tools / Tool plugins 页面复用上传和 Server task 轮询，按 `/api/tools` descriptor 的 `source/tags/readOnly/exportable/runnable` 展示 configured 与 built-in 工具差异；所有 runnable 工具都按 `paramsTemplate` 预填 JSON 参数并允许手工修改后运行，metadata built-ins 不需要上传并展示实例列表、snapshot 摘要或 field/tag field 表，configured command tools 可上传匹配文件或通过 `params.inputFiles` 复用 Workspace 已知输入；`params.inputFiles` 模式要求用户提供已有 Workspace id，并禁止同时选择新上传文件，确保逻辑路径解析落在同一个 Workspace。`logagent.preprocess_log_package` 接受多个 `.tar.gz` 日志包并展示预处理指标、节点表和 tool input index 表，source-built analyzer/configured command tools 展示 findings、stdout/stderr preview 和 stdout/stderr artifact 路径，多输入手动 configured run 展示 aggregate status、inputFiles、per-input results 和 artifactPaths，`logagent.huawei_cloud_package_sync` 上传一个包并展示 OBS/GaussDB 同步结果、query rows 和凭据环境变量名，`pprof_analyzer` 展示 profile type、total、top 函数表和 top/tree/raw/stderr artifact 路径，其他工具展示 JSON result。
...
- System Context 页面内的 `Metadata` 标签页继续提供 Metadata 导入、实例管理、snapshot 查看和 raw JSON 刷新能力。
