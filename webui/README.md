...
- `Analyze` 顶部新增 V2 分析桥接面板，直接调用 Python V2 `/api/v2/*`：可新建 Workspace、选择历史 Workspace、回填并保存 Workspace 问题/模式、软删除历史 Workspace、上传小文件或分片上传大文件、创建 Run、轮询 `/api/v2/runs/:run_id/analysis`，展示 V2 run 状态、evidence、timeline、pending actions、resources、最终结果和 artifacts；当 V2 run 等待用户或审批时，可提交补充、请求基于当前证据收尾、批准或拒绝 action，补充消息返回的 `answeredActions` 会被后续轮询反映到 pending actions，并通过带 Authorization header 的下载按钮读取 `/api/v2/artifacts/:artifact_id`。旧 Session-first Analyze 流仍保留，作为 V2 WebUI 全量切换前的兼容入口。
