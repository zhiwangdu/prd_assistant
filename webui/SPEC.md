...
- Tools 页面必须展示 descriptor 的 `source/tags/readOnly/editable/exportable/runnable` 信息，用 tag 区分 configured 和 built-in；built-in 工具只读、不可编辑、不可导出，不显示上传运行表单。
- Tools 页面首版支持 `pprof_analyzer` 上传 profile、设置 sample index/node count/SVG 开关、创建 `tool_run` task、轮询状态并展示 top 函数表和 artifact 路径；内置 metadata 工具展示输入 schema 和只读能力状态。
- Tools 页面必须提供 `Tool plugins` / `Executors` 子页；Executors 子页来自 `/api/executors`、`/api/executor-command-templates` 和 `/api/executor-runs`，支持执行机新增/编辑/禁用、选择白名单命令模板、创建 remote command run、轮询状态并展示 stdout/stderr preview 和 result/stdout/stderr artifact 路径。
