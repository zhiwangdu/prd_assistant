...
- 成功任务 artifacts 中存在 `analysisPackage`、`agentRequest` 或 `agentResponse` 时，WebUI 必须展示 Agent contract 面板，包含默认后端、执行模式、运行状态和三份契约路径。
...
- 带 Agent contract artifacts 的任务能展示 `analysis_package.json`、`agent_request.json`、`agent_response.json` 路径和 `not_invoked` 状态。
