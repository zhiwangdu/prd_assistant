...
- V2 Executors bridge 必须调用 `/api/v2/executor-command-templates` 展示白名单命令模板，调用 `/api/v2/executor-runs` 创建和列出 remote command run，轮询 `/api/v2/executor-runs/:run_id` 并在成功后读取 `/api/v2/executor-runs/:run_id/result` 展示 stdout/stderr preview 和 result/stdout/stderr path。
