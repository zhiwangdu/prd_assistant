...
- V2 System Context 页面必须调用 `/api/v2/system-context/resources` 展示 V1-compatible System Context resource 和只读 Metadata adapter 摘要；对 `source=system_context` 的资源必须支持创建、读取详情、编辑标题/描述/scope/enabled/tags/product/version/environment、追加版本、激活版本，并调用 `/api/v2/system-context/preview` 预览显式选择的资源或 `meta_<instanceId>` adapter 及后端 prompt。
