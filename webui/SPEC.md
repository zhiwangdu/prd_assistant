...
- V2 Tools 页面选择成功的 `pprof_analyzer` 手动 `tool_run` 后必须识别
  result 中的 `toolId=pprof_analyzer`、`profileType`、`sampleIndex`、`total`、
  `top[]` 和 `artifactPaths/artifacts`，展示状态指标、top 函数表、错误/警告
  以及 top/tree/raw/stderr/SVG artifact 逻辑路径；其他工具继续展示 JSON
  result。
