...
- 顶部导航默认选中 `Log Analysis`，可见顺序必须是 `Log Analysis`、`Memory`、`System Context`、`Tools`、`Settings`。
...
- `Task execution` 必须实时轮询 `/api/tasks/:task_id/analysis`，展示 loop revision、预算、最近事件、backend callId/attempt、model decision、action 和 evidence 摘要。
...
- 成功任务 artifacts 中存在 `analysisPackage`、`agentRequest` 或 `agentResponse` 时，WebUI 必须展示 Claude Code backend 面板，包含默认后端、执行模式、真实运行状态、耗时、错误和三份 artifact 路径。
...
- Settings 页面必须展示 Agent Backends：读取默认后端、后端类型、启用状态、执行模式和 dry-run 诊断结果；第一阶段外部 adapter 诊断只检查配置路径，不执行命令。
...
- mock Claude SDK adapter 任务能展示 Agent 最终结果，adapter 失败能展示 `PLAN_ANALYSIS` 错误。
...
- 带 Agent backend artifacts 的任务能展示 `analysis_package.json`、`agent_request.json`、`agent_response.json` 路径、真实 runtime 状态、usage/cost、耗时和错误。
- mock Claude SDK adapter 任务能完成追问、审批、恢复和最终结果展示。
