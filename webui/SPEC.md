...
- V2 Tools 页面必须调用 `/api/v2/tools` 展示 V2 工具目录和
  ...
  registered/enabled/runnable、command path、command exists、command executable、
  timeout、max input files 和 unavailable reason，并支持通过带 Authorization
  header 的请求下载 `/api/v2/exports/tools.zip`。
