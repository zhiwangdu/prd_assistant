...
- V2 Tools 页面的 Agent 调用必须支持 run-scoped：用户填写 V2 `run_id` 后，前端调用 `/api/v2/mcp/task/:run_id` 的 JSON-RPC `tools/call`。配置工具使用 `logagent.run_domain_tool`，`logagent.fetch` 使用 `logagent.fetch`；响应直接展示 MCP result/error。
