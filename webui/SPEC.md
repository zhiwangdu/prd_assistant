...
- Topology 不渲染 Graph，而是派生可筛选、可级联展开的 PT 视图：
...
- Databases 页面必须按 `Database -> RP -> ShardGroup/IndexGroup -> Shard/Index` 级联展开，不默认铺开全部明细表。
- Schemas 页面必须提供 Database、RP、Measurement/field 筛选；未输入筛选条件时不铺开全部 Schema。
...
- Topology 在大集群下通过 `Database -> DataNode -> DBPT -> Shards` 级联展开可用，不渲染 Graph。
- Databases 页面按 Database/RP/ShardGroup/IndexGroup 层级折叠，展开后能看到 Shard 和 Index 明细。
- Schemas 页面有筛选条件时才展示匹配 Schema，避免默认铺满全部表结构。
