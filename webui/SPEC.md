...
提供 System Context 管理、日志上传、调查交互、证据查看和 openGemini Metadata 可视化。前端使用 React + Vite + TypeScript + Tailwind CSS，shadcn/ui 负责基础交互组件，React Flow 负责拓扑图。
...
- Topology 默认不渲染全量实体图，而是先派生异常优先的 PT 聚合概览：

```text
DataNode / Database / PT
  -> ShardGroup count / Shard count
  -> IndexGroup count / Index count
  -> Diagnostic count / time range
```

- Focused Graph 在用户选择具体 PT 后渲染小范围关系图：
...
- Focused Graph 超过 600 个图元素时必须阻止渲染，并提示用户关闭 Shard/Index 或缩小筛选范围。
- 点击实体展示完整字段和关联对象。
- Diagnostics。
...
- Topology 默认概览在大集群下可用，不要求渲染全量 React Flow 节点。
- 选择具体 PT 后 React Flow Focused Graph、Diagnostics 和 Raw JSON 可用。
- Focused Graph 超过 600 个图元素时显示缩小范围提示，不渲染大图。
