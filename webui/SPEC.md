...
- Vite 开发服务默认把 `/api` 和 `/health` 代理到 Python V2
  ...
  `VITE_LOGAGENT_API_TARGET` 临时覆盖到其它后端。
- Python V2 Server 使用 `LOGAGENT_V2_WEBUI_DIR` 或默认仓库
  `webui/out` 托管静态页面。
- `scripts/build-webui.sh` 构建 `webui/out`，并可通过 `--output-dir` 或
  `LOGAGENT_WEBUI_OUT_DIR` 同步到指定静态目录。
- API Key 保存在本机 localStorage。首次打开本地开发地址
  (`127.0.0.1`、`localhost` 或 `::1`) 且 localStorage 没有已保存 API Key
  时，前端必须自动使用本地脚本默认的 `dev-token`；非本地地址不得自动填充
  API Key。
...
- Metadata 页面能上传 `.json` / `.yaml` / `.csv` 文件生成导入预览并确认写入，选择 `.csv` 文件后自动使用 `templateType=csv`。
- Metadata 页面能粘贴 JSON/YAML/CSV 文本生成导入预览并确认写入。
...
- V2 Tools 页面必须调用 `/api/v2/tools` 展示 V2 工具目录和
  `sourceBuiltAnalyzers` 状态。工具目录必须优先使用 `tools` 字段，并兼容
  同内容的 `toolPlugins` alias；二者都缺失时才显示空目录。认证缺失、认证
  失败或 catalog 尚未加载时，空态必须展示当前加载/认证状态，不能只显示
  `No V2 tools`。工具目录展示 enabled、backend、source、tags、readOnly、
  editable、exportable、manualOnly、runnable、文件数量约束、accepted suffixes、
  output views、match、params template 和 params schema；source-built analyzer 状态必须展示四个 submodule analyzer 的
  registered/enabled/runnable、command path、command exists（`exists/no file`）、
  command executable（`exec/no exec`）、timeout、max input files 和 unavailable
  reason，并支持通过带 Authorization header 的请求下载
  `/api/v2/exports/tools.zip`。
- V2 Tools 页面的 Agent 调用必须支持 run-scoped：用户填写 V2 `run_id` 后，前端调用 `/api/v2/mcp/task/:run_id` 的 JSON-RPC `tools/call`。configured command/source-built analyzer 使用 `logagent.run_domain_tool`，metadata built-ins 使用同名 MCP tool，`logagent.fetch` 使用 `logagent.fetch`；manual-only 工具例如 `logagent.preprocess_log_package`、`pprof_analyzer` 和 `logagent.huawei_cloud_package_sync` 不能误路由到 `logagent.run_domain_tool`，前端必须提示改用独立 Manual `tool_run`。响应直接展示 MCP result/error。
- V2 Tools 页面必须支持独立手动 `tool_run`：用户可填写已有
  Workspace id，或留空由前端调用 `/api/v2/workspaces` 自动创建工具运行
  Workspace；有文件输入约束的工具可通过 Workspace upload API 上传匹配文件，
  也可在工具 schema/template 暴露 `inputFiles` 时通过 Params JSON 的
  `inputFiles` 复用 Workspace 已知输入；`inputFiles` 非空时前端必须要求
  已有 Workspace id，并禁止同时选择新上传文件，避免自动创建空 Workspace
  后引用不存在的逻辑路径；然后调用 `/api/v2/tools/:tool_id/runs` 创建 run，使用
  `/api/v2/tools/runs` 刷新历史并通过 `/api/v2/tools/runs/:run_id/result`
  读取结果。
- V2 Tools 页面选择手动 `tool_run` 后必须调用
  `/api/v2/tools/runs/:run_id/artifacts` 展示 uploads、evidence artifacts 和
  support artifacts；非终态选中 run 必须定时刷新状态，终态后展示 result JSON，
  artifact 下载必须通过携带 Authorization header 的
  `/api/v2/artifacts/:artifact_id` 完成。
- V2 Tools 页面选择成功的 `logagent.preprocess_log_package` 手动
  `tool_run` 后必须识别 response 的 `resultPath`，以及 result 中的
  `toolId=logagent.preprocess_log_package`、`summary`、upload/file/node/tool
  input 计数、`manifestPath`、`grepResultsPath`、`toolInputsPath`、`logGroups`、
  `nodes` 和 `toolInputIndex`，展示状态指标、artifact 逻辑路径、log group
  汇总、节点聚合表和 materialized tool input 表；该视图仍必须保留 raw JSON。
- V2 Tools 页面选择成功的 metadata built-in 手动 `tool_run` 后必须识别
  `logagent.list_metadata_instances`、`logagent.get_metadata_snapshot`、
  `logagent.get_metadata_field_types` 和 `logagent.get_metadata_tag_fields`，
  展示实例列表表格、snapshot 摘要或 field/tag field 表，同时保留 params 和
  raw JSON。
- V2 Tools 页面选择成功的 configured command 或 source-built analyzer 手动
  `tool_run` 后必须识别 `schemaVersion=2`、`toolId` 和 `findings[]`，展示
  status、exit code、timeout、duration、input file、result/stdout/stderr
  逻辑路径、findings 表、stdout/stderr preview、parsed stdout 和 raw JSON。
- V2 Tools 页面选择成功的多输入 configured command 或 source-built analyzer
  手动 `tool_run` 后必须识别 `schemaVersion=1`、`toolId` 和 `results[]`
  聚合结果，展示 aggregate status、inputFiles、per-input results、
  per-input artifact paths、artifactPaths、params 和 raw JSON。
- V2 Tools 页面选择成功的 `logagent.huawei_cloud_package_sync` 手动
  `tool_run` 后必须识别 OBS PUT/HEAD、GaussDB update/query、object key/url、
  SQL metadata、timings、bounded query rows、warning/error 和 credential
  environment metadata，并保留 raw JSON。
- V2 Tools 页面选择成功的 `pprof_analyzer` 手动 `tool_run` 后必须识别
  response 的 `resultPath`，以及 result 中的 `toolId=pprof_analyzer`、
  `profileType`、`sampleIndex`、`total`、`top[]` 和 `artifactPaths/artifacts`，
  展示状态指标、top 函数表、错误/警告以及 result/top/tree/raw/stderr/SVG
  artifact 逻辑路径；其他工具继续展示 JSON result。
- V2 Tools 页面选择工具时必须用 descriptor 的 `paramsTemplate` 预填 Params JSON，避免用户手工猜测内置工具和 submodule analyzer 的参数结构。
- V2 Tools 页面手动上传文件选择器必须把 descriptor `acceptedSuffixes`
  中的 `.ext` 和 `*.ext` 转换为 HTML `accept` 可识别的扩展名，并忽略
  `*keyword*` 这类非扩展 file pattern；工具详情仍展示后端返回的原始
  `acceptedSuffixes`。
- V2 Tools 页面不提供旧 Rust Tool Runner 的独立上传型 `/api/tools/:tool_id/runs` 兼容入口；独立手动运行必须走 V2 `/api/v2/tools/:tool_id/runs`，工具产物挂在对应 V2 run 的 evidence/artifacts 上。
...
- Tools 页面支持所有 `/api/tools` catalog 中的 runnable 工具：按 `paramsTemplate` 预填 JSON textarea，用户手工修改后创建 `tool_run` task、轮询状态并展示结果；metadata built-ins 不需要上传并展示实例列表、snapshot 摘要或 field/tag field 表；configured command tools 在 Params JSON 提供 `inputFiles` 时不应强制上传新文件，且必须要求已有 Workspace id 并禁止混用新上传文件；`logagent.preprocess_log_package` 作为 built-in runnable tool 支持批量 `.tar.gz` 日志包上传并展示结构化预处理指标、节点聚合和 tool input index；source-built analyzer/configured command tools 展示 findings、stdout/stderr preview 和 stdout/stderr artifact 路径，多输入聚合结果展示 inputFiles、per-input results 和 artifactPaths；`logagent.huawei_cloud_package_sync` 启用时要求上传一个包并填写 `updateSql` / `querySql`，并展示 OBS/GaussDB 同步结果、query rows 和 credential env metadata；`pprof_analyzer` 保留 top 函数表和 artifact 路径专用展示，其他工具展示 JSON result。
...
- V2 Fetch 页面必须支持 standalone Fetch `tool_run`：用户可填写已有
  Workspace id，或留空由后端自动创建隔离 Workspace；前端调用
  `/api/v2/fetch/endpoints/:endpoint_id/runs` 创建 run，调用
  `/api/v2/fetch/runs` 刷新历史，非终态选中 run 必须定时刷新状态，成功后读取
  `/api/v2/tools/runs/:run_id/result` 和
  `/api/v2/tools/runs/:run_id/artifacts`。
- V2 Fetch 结果面板必须展示 result artifact、response body artifact、
  evidence summary 和 body preview；下载 `result.json` 与
  `response_body.bin` 必须通过携带 Authorization header 的
  `/api/v2/artifacts/:artifact_id` 完成。
...
- 顶部导航能进入 `System Context` 页面；该页是 V2 System Context + Metadata 原生入口，页内通过 `System Context` / `Metadata` 标签页切换模块，默认直接调用 `/api/v2/skills*`、`/api/v2/system-context/*` 和 `/api/v2/metadata*`，不再默认渲染旧 Rust Skills/Metadata 面板。
...
- V2 System Context 页面必须调用 `/api/v2/system-context/resources` 展示 V1-compatible System Context resource 和只读 Metadata adapter 摘要；对 `source=system_context` 的资源必须支持创建、读取详情、编辑标题/描述/scope/enabled/tags/product/version/environment、追加版本、编辑已有版本、激活版本，并调用 `/api/v2/system-context/preview` 预览显式选择的资源或 `meta_<instanceId>` adapter 及后端 prompt。
- V2 System Context 页面必须调用 `/api/v2/metadata/instances` 展示 V2 Metadata instance 摘要。
- V2 Metadata 页面必须调用 `/api/v2/metadata/imports/preview` 和 `/api/v2/metadata/imports/fetch/preview` 支持 JSON/YAML/CSV/openGemini 内容或 URL 的导入预览，并调用 `/api/v2/metadata/imports/:import_id/confirm` 确认写入。
