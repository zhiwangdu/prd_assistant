...
- V2 Tools 页面必须调用 `/api/v2/tools` 展示 V2 工具目录和
  ...
  同内容的 `toolPlugins` alias；二者都缺失时才显示空目录。工具目录展示
  enabled、backend、source、tags、readOnly、editable、exportable、manualOnly、
  runnable、文件数量约束、accepted suffixes、output views、match、params
  template 和 params schema；source-built analyzer 状态必须展示四个 submodule analyzer 的
