...
- V2 Metadata bridge 必须支持直接调用 `/api/v2/metadata/imports` 和 `/api/v2/metadata/imports/fetch` 写入实例，调用 `/api/v2/metadata/imports` 列出导入历史，调用 `/api/v2/metadata/instances`、`/api/v2/metadata/instances/:instance_id/snapshot` 和 `DELETE /api/v2/metadata/instances/:instance_id` 管理实例和查看 snapshot。
