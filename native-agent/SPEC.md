...
- 默认通过 `native_agent.server_api=v2` 连接 `server-v2`；`v1` 仅作为外部旧服务兼容模式保留。
...
- `POST /api/v2/uploads/:upload_session_id/chunks?offset=<bytes>`
- `POST /api/v2/uploads/:upload_session_id/complete`

V2 小文件上传和分片完成接口直接在 Session workspace 下创建 upload，不需要额外 attach。
...
- `server_api=v2` 时，Native Agent 能先创建或复用 `ws_...` Session，再通过 `/api/v2/sessions/:session_id/uploads*` 上传。
