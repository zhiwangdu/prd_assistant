...
**WebUI (`webui/`)** — Tools-first nav with English-only top-level tabs (no bilingual labels): `Tools (with sub-item Runs History) | Skills | MCP | Metadata | Fetch | Executors | Cases | Settings`, default Tools. `RunsView` is now reached via the nested `Runs History` sub-tab under Tools rather than a standalone top-level tab. Views: `ToolsView`, `RunsView`, `FetchView`, `McpView`, `ExecutorsView`, `MetadataDashboard`, `CasesView`, `SkillsView`, `SettingsView`. The legacy `OperationsView` (Analyze) is demoted out of nav pending removal. Vite config rewrites `/api` to the server. All artifact downloads must carry the `Authorization` header; sensitive fields render masked.
