...
class StoreTests(unittest.TestCase):
    ...
    def test_workspace_run_job_and_stub_agent(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(any(item["kind"] == "claude_prompt" for item in evidence))
            self.assertTrue(any(item["kind"] == "claude_mcp_config" for item in evidence))
            self.assertTrue(any(item["kind"] == "claude_session" for item in evidence))
            ...
            claude_session = json.loads(claude_resource["result"]["contents"][0]["text"])
            self.assertEqual(claude_session["runtimeStatus"], "contract_ready")
            self.assertEqual(claude_session["mcpConfigPath"], "claude_mcp_config.json")
            ...
            self.assertEqual(artifacts["claudePromptPath"], "claude_prompt.md")
            self.assertIn("resources/list", artifacts["claudePrompt"]["text"])
            self.assertEqual(artifacts["claudeMcpConfigPath"], "claude_mcp_config.json")
            self.assertEqual(
                artifacts["claudeMcpConfig"]["mcpServers"]["logagent"]["type"],
                "http",
            )
            self.assertEqual(
                artifacts["claudeMcpConfig"]["mcpServers"]["logagent"]["headers"][
                    "Authorization"
                ],
                "Bearer ${LOGAGENT_V2_API_KEY}",
            )
            self.assertEqual(artifacts["claudeSessionPath"], "claude_session.json")
            self.assertEqual(artifacts["claudeSession"]["runtimeStatus"], "contract_ready")
    ...
    def test_agent_token_budget_returns_budget_limited_result(self) -> None:
        captured_prompts: list[dict] = []

        class TokenBudgetProviderHandler(BaseHTTPRequestHandler):
            def do_POST(self) -> None:
                length = int(self.headers.get("Content-Length", "0"))
                payload = json.loads(self.rfile.read(length).decode("utf-8"))
                prompt = json.loads(payload["messages"][1]["content"])
                if prompt.get("task") == "run_alias":
                    answer = {"alias": "Token budget limited"}
                    body = json.dumps(
                        {"choices": [{"message": {"content": json.dumps(answer)}}]}
                    ).encode("utf-8")
                    self.send_response(200)
        ...
        server = HTTPServer(("127.0.0.1", 0), TokenBudgetProviderHandler)
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                state = json.loads(state_response["result"]["contents"][0]["text"])
                self.assertEqual(
                    state["rounds"][0]["tokenUsage"]["totalTokens"],
                    12,
                )
                self.assertEqual(state["rounds"][-1]["status"], "budget_limited")
        finally:
    ...
    def test_agent_user_prompt_budget_returns_budget_limited_result(self) -> None:
        captured_prompts: list[dict] = []

        class UserPromptBudgetProviderHandler(BaseHTTPRequestHandler):
            def do_POST(self) -> None:
                length = int(self.headers.get("Content-Length", "0"))
                payload = json.loads(self.rfile.read(length).decode("utf-8"))
                prompt = json.loads(payload["messages"][1]["content"])
                if prompt.get("task") == "run_alias":
                    answer = {"alias": "User prompt budget limited"}
                    body = json.dumps(
                        {"choices": [{"message": {"content": json.dumps(answer)}}]}
                    ).encode("utf-8")
                    self.send_response(200)
        ...
        server = HTTPServer(("127.0.0.1", 0), UserPromptBudgetProviderHandler)
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                self.assertEqual(waiting_result["status"], "waiting_for_user")
                store.append_event(
                    workspace["id"],
                    run["id"],
                    "user.message",
                ...
                self.assertEqual(
                    final_answer["terminationReason"],
                    "user prompt budget exhausted: 1/1",
                )
                self.assertEqual(store.get_run(run["id"])["status"], "succeeded")
                self.assertEqual(len(captured_prompts), 1)
        finally:
    ...
    def test_agent_repeated_tool_fingerprint_returns_budget_limited_result(self) -> None:
        ...
        class RepeatedToolProviderHandler(BaseHTTPRequestHandler):
            def do_POST(self) -> None:
                length = int(self.headers.get("Content-Length", "0"))
                payload = json.loads(self.rfile.read(length).decode("utf-8"))
                prompt = json.loads(payload["messages"][1]["content"])
                if prompt.get("task") == "run_alias":
                    answer = {"alias": "Repeated action blocked"}
                    body = json.dumps(
                        {"choices": [{"message": {"content": json.dumps(answer)}}]}
                    ).encode("utf-8")
                    self.send_response(200)
        ...
        server = HTTPServer(("127.0.0.1", 0), RepeatedToolProviderHandler)
        thread = threading.Thread(target=server.serve_forever, daemon=True)
        thread.start()
        try:
            with tempfile.TemporaryDirectory() as tmp:
                settings = Settings(
                    ...
                    agent_max_llm_calls=4,
                    agent_max_actions=6,
                    agent_max_repeated_action_fingerprints=1,
                )
                settings.ensure_dirs()
                store = Store(settings.sqlite_path)
                store.initialize()
                ...
                self.assertTrue(final_answer["budgetLimited"])
                self.assertTrue(
                    final_answer["terminationReason"].startswith(
                        "repeated action fingerprint blocked: mcp_tool:"
                    )
                )
                self.assertEqual(store.get_run(run["id"])["status"], "succeeded")
                self.assertEqual(len(captured_prompts), 2)
    ...
    def test_preprocess_tool_run_materializes_node_package_inputs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result["manifestPath"], "manifest.json")
            self.assertEqual(result["grepResultsPath"], "grep_results.json")
            ...
            self.assertEqual(result["toolInputs"], result["toolInputIndex"])
