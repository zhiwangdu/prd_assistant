...
- Log Analyzer 会自动识别节点日志包 `<packageId>_<instanceId>_<nodeId>_<timestamp>_logs.tar.gz`，按 `extracted/<nodeId>/<timestamp>/{tsdb,stream,agent}/` 展开三类日志目录；archive 内允许存在顶层包装目录和 `./` 等目录项，只要文件路径中包含 `var/chroot/gemini/log/{tsdb,stream}` 或 `home/Ruby/log` 即可归类。轮转 gzip 透明读取，并生成 `tool_inputs/index.json`、通用 `log_text` JSONL 和 analyzer-ready 输入；只有匹配当前 `toolIds` 的 materialized input 会被对应工具作为独占自动输入消费。
