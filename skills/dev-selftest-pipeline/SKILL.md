...
| 4. run tests | `logagent.dev_selftest.run_tests` | `{runId, testSuite}`. Dispatch priority: a suite with `executor` (a managed docker-kind executor record id) dispatches through that record; else a suite with a `docker` block dispatches through the executor docker runner (`docker run --rm --network host <image> <argv>`); else the P1 local stub (runs `argv` on the server host). `executor` and `docker` are mutually exclusive. Either way `DEVSELFTEST_HOST`/`DEVSELFTEST_PORT` are injected as system env (final priority). Returns `{status, exitCode, executor, stdoutPath, stderrPath}`. |
...
- **P2 Docker executor 纳管 (done)**: `RemoteExecutorRecord` supports `kind: docker` (+ a
  `docker` spec); `/api/executors` create/patch accept docker fields; `/api/executor-runs`
  runs a docker-kind executor through `run_executor_command` and records it in run history;
  `remote_execution.executors` seeds records at startup. `run_tests` can reference a managed
  docker-kind executor by id (`suite.executor`) — the "纳管 + 指定执行" path (mutually
  exclusive with inline `docker`).
- **P2 still deferred**: parameterized executor command templates (`{var}` + small JSON
  Schema); `ssh_binary_replace` deploy profile + controlled SCP (deploy the built binary to
  a GeminiDB Influx node, swap + restart, health check, rollback) — also covers ssh-kind
  executor test dispatch.
