...
async fn read_json_case_import_input(
    ...
    Ok(CaseImportInput {
        ...
        text: validate_case_import_text(req.text, state.config.llm.max_input_chars)?,
    })
}
...
async fn read_multipart_case_import_input(
    ...
    if let Some(value) = file_text {
        return Ok(CaseImportInput {
            ...
            filename: file_name,
            text: validate_case_import_text(value, state.config.llm.max_input_chars)?,
        });
    }
...
async fn apply_case_import_extraction(
    ...
    let extraction = state
        .llm
        .extract_case_import(&session.source_text, &session.messages)
        .await
        .map_err(|err| AppError::internal(format!("failed to extract case import: {err}")))?;
    let mut draft = extraction.structured_case;
    ...
    let assistant_question = if missing_fields.is_empty() {
        ...
        clean_optional(extraction.assistant_question)
            .or_else(|| default_assistant_question(&missing_fields))
    };
...
fn validate_case_import_text(value: String, max_chars: usize) -> Result<String, AppError> {
    ...
    if chars > max_chars {
        return Err(AppError::bad_request(format!(
            "case import text contains {chars} chars and exceeds llm.max_input_chars {max_chars}"
        )));
    }
    Ok(value)
}
...
fn max_case_import_bytes(state: &AppState) -> usize {
    let llm_bytes = state.config.llm.max_input_chars.saturating_mul(4).max(1024);
