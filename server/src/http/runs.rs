...
pub async fn list_runs(
        ...
        .filter(|value| !value.is_empty())
    {
        ...
        Some("log_analysis") => Some(TaskKind::LogAnalysis),
        Some(other) => {
...
fn result_path_for(task: &TaskRecord) -> Option<String> {
    match task.task_kind {
        ...
        TaskKind::LogAnalysis => task.result_json_path.clone(),
    }
}
...
mod tests {
    ...
    use crate::{
        domain::models::{AnalysisLanguage, TaskKind, TaskRecord, TaskSource, TaskStatus},
        http,
        support::config::{
            ...
            ServerSettings, SkillSettings, StorageSettings, ToolsSettings,
        },
    };
