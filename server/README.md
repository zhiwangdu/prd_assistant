...
每个 Session 持久化到 `sessions/<session_id>.json`，事件追加到 `session_workspaces/<session_id>/session_events.jsonl`。每个任务持久化到 `tasks/<task_id>.json`。写入使用同目录临时文件加 rename；启动时任何损坏的 Session/任务 JSON 都会导致 Server 明确启动失败。
