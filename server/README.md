...
Server 是任务管理、分析调度和内部证据能力的统一承载进程。当前 MVP 不把日志解析、工具执行、Metadata、Analysis Orchestrator、Agent Backends、Domain Adapters、LLM Gateway 或 Memory/Case Store 拆成独立 crate / 服务；这些能力都在 `server` crate 内部分层实现，后续确有独立生命周期或部署需求时再迁出。
...
- 编排 Log Analyzer、Tool Runner、Code Evidence、Environment Collector、Analysis Orchestrator 和 Agent Backend Adapter
- 持久化分析上下文、事件、预算、待回答问题和待审批动作
- 校验并执行 Agent Backend 返回的结构化动作
- 管理模块输出和任务失败原因
- 查询和关联实例、集群、节点元数据
- 管理 System Context 通用背景资源、Prompt Pack、架构文档和 Metadata adapter
- 用户消息、动作批准/拒绝和任务恢复
...
- Services：Log Analyzer、Tool Runner、Metadata、Agent Backend、Domain Adapter、LLM Gateway、Tools 插件等内部能力。
...
后续新增 Code Evidence、Environment Collector 或扩展 Agent Backend / Domain Adapter 时，默认继续放在 `services/`、`pipeline/`、`stores/` 或 `domain/` 的对应层内，只有出现明确的独立发布、复用或部署边界时才拆 crate。
...
- `POST /api/system-context/preview` 预览当前筛选条件下将注入 Prompt 的 System Context。
...
- Task 创建时固化 `metadata_context.json`、`system_context.json` 和 `case_context.json`，同时向 Session timeline 追加 Metadata summary、System Context resource count 和 Case recall count。
