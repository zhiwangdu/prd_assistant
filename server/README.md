...
Server 是任务管理、分析调度和内部证据能力的统一承载进程。当前 MVP 不把日志解析、工具执行、Metadata、Analysis、LLM Gateway 或 Memory/Case Store 拆成独立 crate / 服务；这些能力都在 `server` crate 内部分层实现，后续确有独立生命周期或部署需求时再迁出。
