...
- 内网环境无法访问 GitHub 时，可在运行 `scripts/build-tools.sh` 前设置 `LOGAGENT_SUBMODULE_BASE_URL`，或分别设置 `LOGAGENT_SUBMODULE_FLUX_URL`、`LOGAGENT_SUBMODULE_INFLUXQL_URL`、`LOGAGENT_SUBMODULE_OPENGEMINI_URL`、`LOGAGENT_SUBMODULE_INFLUXDB_URL`。构建脚本会把这些地址写入本地 Git submodule config，再按需初始化 `third_party/` 源码。
