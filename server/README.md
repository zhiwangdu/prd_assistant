...
- 识别节点日志包并生成按节点/时间/日志类型组织的解压目录和 analyzer-ready `tool_inputs`；节点包 archive 内可带顶层包装目录
...
- 匹配节点日志包命名的 `.tar.gz` 会进入专用预处理；archive path 可带顶层包装目录，Server 会在路径中查找 `var/chroot/gemini/log/{tsdb,stream}` 和 `home/Ruby/log` 并展开到 `extracted/<nodeId>/<timestamp>/{tsdb,stream,agent}/`。如果一个节点包没有任何支持日志目录，EXTRACT phase 会失败并返回明确错误，不生成空 manifest。
