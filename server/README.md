...
- 管理 Fetch endpoint、加密 credential set、DevTools cURL 导入、受控 HTTP 出网和响应 artifact
- 识别节点日志包并生成按节点/时间/日志类型组织的解压目录和 analyzer-ready `tool_inputs`；节点包 archive 内可带顶层包装目录和目录项
...
- Services：Log Analyzer、Tool Runner、Fetch、Huawei Package Sync、Metadata、Claude Code Session Runner、Domain Adapter、LLM Gateway、Tools 插件等内部能力。
...
- `source`: `upload` / `environment` / `remote_executor`
...
- `POST /api/mcp/readonly` 提供面向个人本地 Claude Code 的只读 HTTP MCP，支持读取 Skills、Metadata、Metadata field type/tag fields、Case、Tools catalog 和 Domain Adapter 摘要，不操作 Session/task/workspace，也不执行 Fetch endpoint。
- Fetch endpoint API：`POST /api/fetch/imports/preview` 解析 DevTools bash cURL 并返回脱敏预览；`GET/POST /api/fetch/endpoints`、`GET/PATCH/DELETE /api/fetch/endpoints/:fetch_id` 管理 endpoint；`POST /api/fetch/endpoints/:fetch_id/runs` 创建 `tool_run`；`GET /api/fetch/runs?fetchId=...` 查询最近 fetch run。
...
- 匹配节点日志包命名的 `.tar.gz` 会进入专用预处理；archive path 可带顶层包装目录，`./` 或普通目录项会被跳过，Server 会在文件路径中查找 `var/chroot/gemini/log/{tsdb,stream}` 和 `home/Ruby/log` 并展开到 `extracted/<nodeId>/<timestamp>/{tsdb,stream,agent}/`。如果一个节点包没有任何支持日志目录，EXTRACT phase 会失败并返回明确错误，不生成空 manifest。
...
- 每次 Session 创建 task run 时都会写入 task `sessionId` 和 `analysisLanguage`，并把 taskId 追加到 Session `taskIds`，更新 `activeTaskId/status`。
...
- `RUN_TOOL` 阶段对已配置工具生成规则版 `run_tool` action；如果 `tool_inputs/index.json` 中存在匹配当前 toolId 的 materialized input，则只选择这些 tool-ready 输入并受 `max_input_files` 限制，不再补充原始日志候选。没有匹配 materialized input 时才按 manifest file pattern 优先、grep keyword 补充候选；未匹配或未配置工具时直接进入 `PLAN_ANALYSIS`。
...
- Fetch endpoint 默认关闭；启用 `fetch.enabled=true` 时必须配置 `fetch.secret_key_env` 指向 32-byte base64 密钥，并配置 `fetch.allowed_hosts`。Server 用 AES-256-GCM 加密保存 Authorization、Cookie、token/api_key 类 query/body 字段；所有 API/UI/log/artifact 只展示 `<redacted>`。
- Fetch 执行使用 `reqwest`，每次请求和 redirect hop 都校验 `scheme/host/port` allowlist；跨 host redirect 不转发 Authorization/Cookie，拒绝用户导入或覆盖 `Host`、`Content-Length`、`Transfer-Encoding`、`Connection` 等受控 header。HTTP 4xx/5xx 记录为完成请求，DNS/timeout/allowlist/解析错误才让 task failed。
- 任务 MCP 新增 `logagent.list_fetch_endpoints` 和 `logagent.fetch`；后者写入 `tool_results/<action_id>/result.json` 和 `response_body.bin`，最终结果只允许引用 `tool_results/<action_id>/result.json#response` 且 action 必须是当前任务真实 `tool=logagent.fetch`。
- Huawei package sync 默认关闭；启用 `huawei_cloud.package_sync.enabled=true` 后，Tools 目录出现 runnable `logagent.huawei_cloud_package_sync`。该工具只支持手动 `tool_run`，必须引用一个已上传文件，按 `objectKey` 或配置 `object_prefix + filename` PUT 到 Huawei OBS，随后按用户 JSON 参数执行 GaussDB update/query SQL，并把 OBS PUT/HEAD、GaussDB affected rows、bounded query rows、耗时和凭据环境变量名写入 `tool_results/<action_id>/result.json`。OBS/GaussDB 密钥仅从环境变量读取，不写入结果、日志或工具目录。
...
- 内网环境无法访问 GitHub 时，可在运行 `scripts/build-tools.sh` 前设置 `LOGAGENT_SUBMODULE_BASE_URL`，或分别设置 `LOGAGENT_SUBMODULE_FLUX_URL`、`LOGAGENT_SUBMODULE_INFLUXQL_URL`、`LOGAGENT_SUBMODULE_OPENGEMINI_URL`、`LOGAGENT_SUBMODULE_INFLUXDB_URL`。构建脚本会把这些地址写入本地 Git submodule config，再按需初始化 `third_party/` 源码；未初始化的 submodule 目录不会触发 `remote set-url origin`，避免改写顶层仓库 remote。
...
- `RUN_TOOL` 后进入 `PLAN_ANALYSIS`。分析前会刷新 `analysis_package.json`、`claude_prompt.md` 和 `claude_mcp_config.json`，随后调用 Claude Code CLI，并把 session 信息写入 `claude_session.json`、MCP 调用写入 `mcp_calls.jsonl`、真实响应写入 `agent_response.json`。`LOGAGENT_CLAUDE_CODE_PATH` 可直接指向 Claude Code CLI `claude` 二进制；Server 会使用 `--print --output-format json --json-schema ... --mcp-config ... --strict-mcp-config` 调用并解析 CLI envelope。Server 自动注入 `--allowedTools mcp__logagent__*`，保证 `dontAsk` 模式下任务 MCP tools 可直接使用；`diagnose` 仍通过 `--tools ""` 禁用 native tools。证据包不再内联到 CLI 参数或 stdin；Claude 通过任务 MCP `analysis_package` resource 读取，且 package 内的 Metadata 只保留 `metadataContextOutline`。Claude 输出只能是 `completed`、`waiting_for_user` 或 `waiting_for_approval` structured outcome；完成时直接持久化 `result.json` / `result.md`，等待态复用现有 message/approval API。`resumeMode=finalize` 的用户消息会让 `analysis_package.analysisState.finalizeRequested=true`，prompt 要求 Claude 不再返回 `waiting_for_user`。任务 MCP `logagent.search_logs` 后续搜索写入 `log_searches/logsearch_*.json` 并返回命中行正文、关键词计数和未命中关键词；prompt 要求 Claude 检查 `matches[].text`，不能只按 `totalMatches` 推断异常类型或技术栈。Claude 启动 prompt 同时读取 task `analysisLanguage`：`zh-CN` 要求 finalAnswer、追问和审批原因等自然语言字段优先简体中文，`en-US` 要求英文；API 名、路径、JSON key、工具名、产品名和 evidence refs 保持原值。
