...
当前基础 pipeline 实际产生 `QUEUED`、`RUNNING`、`SUCCEEDED`、`FAILED`，阶段为 `EXTRACT` 和 `SEARCH_LOGS`。`WAITING_FOR_USER`、`WAITING_FOR_APPROVAL` 已保留在模型中供后续 Analysis Agent 使用。
...
- 每次执行先清理 `extracted/`、`manifest.json`、`grep_results.json`，只从 raw 快照重建派生产物。
