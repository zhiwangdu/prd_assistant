...
- 外部 Agent Backend 契约产物：`analysis_package.json`、`agent_request.json`、`agent_response.json`
- task artifact 查询
- metadata 查询和导入确认
...
Settings Agent Backend 诊断接口用于验证成熟 agent 后端适配配置。`GET /api/settings/agent-backends` 返回默认后端、后端类型、启用状态、执行模式、超时和输入输出大小限制；不返回命令路径。当前支持 `internal_llm`、`claude_agent_sdk`、`codex_cli`、`claude_code_cli` 和 `opencode_cli`。`POST /api/settings/agent-backends/:backend_id/test` 使用 `{ok,result,error}` 响应；`internal_llm` 返回 ready，外部 adapter 第一阶段只检查配置命令路径存在且是普通文件，不执行命令。

`PLAN_ANALYSIS` 每轮在调用当前 `internal_llm` 决策前会刷新三份外部后端契约产物：
...
- `agent_response.json`：当前为 `not_invoked` 占位响应，标明外部后端尚未参与主执行路径。
...
- 接入真实 Claude Agent SDK adapter PoC，让 `agent_response.json` 成为实际后端输出并映射到现有 action/final answer schema。
