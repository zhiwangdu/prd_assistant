...
开发自测能力是一组受控 MCP step tools（docker 闭环）。Server 提供 `logagent.dev_selftest.*`（sync_workspace / build / deploy / run_tests / report / cleanup）和持久 run 工作区 `data/dev_selftest/runs/{runId}/`（含 `source/`、`artifacts/`、`logs/`、`progress.json`、`report.md`）；完整 workflow 由外部 MCP client / 本地 skill 串联，Server 不提供 workflow API 或 agent loop。`dev_selftest.enabled=false` 时整组禁用。
