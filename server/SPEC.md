...
- `agent_response.json`：adapter 实际响应记录，包含 `runtimeStatus`、原始 decision、`normalizedDecision`、usage/cost、`durationMs` 和错误信息。
