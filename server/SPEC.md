...
- Claude Code session 的 Metadata 按需加载：`analysis_package.json` 只包含 `metadataContextOutline`，任务 MCP `metadata_context` resource 和 `logagent.get_metadata_topology` 返回 outline，`logagent.query_metadata` 按 section/filter/limit/cursor 写入 `metadata_slices/<stable_id>.json`，`logagent.get_metadata_field_types` 从已导入 Metadata Store 查询指定 measurement 的字段类型
- task artifact 查询
- metadata 查询和导入确认
...
- pipeline 重跑保留 Metadata 快照，Claude package/MCP 默认 resource 只包含 Metadata outline/counts；`logagent.query_metadata` 的 limit/cursor/filter 必须生效，非法 section/filter 必须失败；`logagent.get_metadata_field_types` 必须能按指定 instance/database/measurement 查询一个、多个或全部 field 类型。
