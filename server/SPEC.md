...
`logagent://dev_selftest/config` 返回脱敏摘要：`gitRepos`、`defaultGitRepo`、`defaultGitRef`、`buildProfiles`、`dockerProfiles`、`testSuites`、Docker cluster profile 明细以及 build/test profile 明细（host/docker、image、timeout），供客户端 skill 在调用 `sync_workspace` 前发现真实 allowlist 和 deploy profile，不得猜测或读取 Server 本机配置文件。

## Dev Self-Test

开发自测能力是一组受控 MCP step tools（docker 闭环）。Server 提供 `logagent.dev_selftest.*`（sync_workspace / build / deploy / run_tests / report / cleanup / diagnose）和持久 run 工作区 `data/dev_selftest/runs/{runId}/`（含 `source/`、`artifacts/`、`logs/`、`progress.json`、`report.md`）；完整 workflow 由外部 MCP client / 本地 skill 串联，Server 不提供 workflow API 或 agent loop。`dev_selftest.enabled=false` 时整组禁用。
...
- 当前实现：git-only 源码同步、配置式 build + artifact glob 收集、`docker_cluster` 部署（`docker compose -p … up -d` + 声明式 health check）、规则化 report，以及 report 后显式可选的 cleanup。`sync_workspace` 必须提供 allowlisted `gitRepo` + `gitRef`；新 run 对 `source/` 执行 clone，复用已有 run 时对已有 git checkout 执行 fast-forward pull。health check 失败不做自动回滚，证据写入 logs/report。
...
- `run_tests` 两种模式：带 `docker` 块的测试套件经 **inline Docker runner** 派发（见下）；无 `docker` 块则走本地桩（在 Server 主机跑配置式 `argv`）。`run_tests` 支持 `runMode:"queued"`：返回 `{runId,status:"QUEUED"}` 后用 `logagent.runs.get`/`runs.result` 轮询（platform 工具，不建 ToolRun）。`run_tests` 还支持可选 `testParams` string map，用于外部/internal skill 创建云 openGemini/influxdb 实例后传入 `caseName`、`instanceId`、`endpoint` 等非敏感运行参数；该路径可跳过 `deploy`，不复用 `DevSelftestDeployTarget::Instance`，因为 ToolHub 不拥有云实例生命周期。
...
- **dev_selftest 内联 Docker target**：`run_tests` 对 `docker` 块内联构建 `ExecutorTarget::Docker`（image/network/workdir/volumes/env 来自配置），argv/timeout 取自 `suite.command` 引用的 `remote_execution.commands` 模板（无 `command` 则用 `suite.argv`）。volume host 侧 `${DEVSELFTEST_*}` 经 `deploy_env` 插值并断言插值后绝对。系统 env（`DEVSELFTEST_HOST/PORT` + run 目录 4 var + `DEVSELFTEST_PARAM_*`）**最终优先**，用户 `env` 不可覆盖。`testParams` key 必须匹配安全 ASCII 标识符、value 必须为有界非空字符串；Server 拒绝 secret-like key、归一化 env 名碰撞和超限输入。参数转换规则为 `caseName`/`case_name`/`case-name` → `DEVSELFTEST_PARAM_CASE_NAME`，碰撞直接失败。`--network host` 下 `127.0.0.1:<host 暴露端口>` 即 ts-sql。
- **testParams 安全边界**：`testParams` 不得携带凭据。当前 Docker runner 用 `docker run --env KEY=VALUE`，值在容器启动期间会出现在宿主机进程 argv；日志/result 只保存 `testParamsSummary`（key、envName、valueBytes 和 caseName），`report.json/report.md` 不扩展 per-step 参数。
...
- `deploy` 把 run 目录环境变量（`DEVSELFTEST_RUN_DIR/SOURCE_DIR/ARTIFACTS_DIR/PROJECT_NAME`）和可选 `DEVSELFTEST_PORT`（来自 Docker cluster `exposed_port`）注入 `docker compose` **和** health check 命令，使 compose 可用 `${DEVSELFTEST_SOURCE_DIR}` 挂载本次 run 编译出的二进制，并可按 profile 暴露端口（通用，非 openGemini 专属）。
- `cleanup` 参数为 `runId` 和可选 `profile`；省略 `profile` 时读取 run 的 Docker deploy target。Server 用同一 project name 规则派生 `devselftest_<runId>_<profile>`，执行 `docker compose -p <project> -f <configured compose_file> down`。该 step 不接受任意 project name、compose path、shell string、`--volumes` 或本地路径删除参数；只写入 `logs/cleanup.stdout.txt`、`logs/cleanup.stderr.txt`、`progress.json` 和 tool result，保留 `source/`、`artifacts/`、`report.*` 等审计证据。cleanup 失败只记录为 cleanup step 失败，不影响 report 的核心自测 verdict。
- `diagnose` 参数为 `runId`，可选 `taskRunId`、`step`、`profile`、`includeDockerProbes`、`maxEvidenceBytes`。Server 只读取本 run 目录内的 `progress.json`、`report.json` 和 step evidence，返回 bounded tail 且脱敏；Docker probe 只允许基于配置化 cluster profile 派生固定只读命令：`docker compose ps`、`docker compose logs --tail`、按 compose project label 的 `docker ps`，以及按 exposed port 的 `docker ps`。输出包含原因分类（端口冲突、残留 compose project、compose up 失败、health check 失败、容器异常、build/test/cleanup 失败等）、证据片段和建议；不得执行 cleanup、restart、rm 或任意 shell。
...
- Docker 路径已对真实 **openGemini** 3meta+3(sql+store) 集群端到端跑通（sync→build→deploy→run_tests→report，全 SUCCEEDED；cleanup 可选释放 compose 资源）。集群 artifact（compose/模板/entrypoint/build 脚本）作为默认 demo 纳入仓库 `deploy/devselftest/opengemini/`，单模板 + entrypoint 按 `OG_ADDR/OG_ID/OG_META_*` env 替换占位符。内网可配置（经 server 进程 env，无代码改动）：`OG_BASE_IMAGE` 换镜像名、`GOPROXY/GOSUMDB` 换 Go 模块源、`dev_selftest.git.repos` 换 openGemini 源码镜像。关键约束：容器需**静态 IP**（openGemini raft 用 `rpc-bind-address` 串作 Server ID，主机名会与绑定 IP 不匹配导致不选主）、`ubuntu:24.04`（22.04 libstdc++ 过旧）、顺序启动门控（meta→store→sql，`depends_on` 仅排序，entrypoint 须等就绪）、store 探活用容器自身 IP 而非 127.0.0.1。
- Docker 路径新增 **InfluxDB OSS v1** 单机 demo：`deploy/devselftest/influxdb/` 提供 Docker-backed build profile 脚本、单容器 compose、`influxdb.conf` 和 v1 HTTP smoke。默认 git repo/ref 为 `ssh://git@github.com/zhiwangdu/influxdb.git` + `master-1.x`，只构建 `build/influxd`，不引入集群或企业能力。默认 builder/runtime/test images 分别为 `golang:1.26-bookworm`、`ubuntu:24.04`、`alpine:3.20`，可通过 probe/script/env 替换为内网镜像。
