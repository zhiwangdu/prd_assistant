...
- V2 LangGraph Agent runtime 已按 V1 默认值实现最大轮次、最大 LLM 调用、最大动作数、重复工具 fingerprint、总 token、单次运行时间和用户追问预算终止；预算耗尽或重复动作触发会停止继续调用 provider / 跳过重复工具执行，生成带 `budgetLimited=true` 和 `terminationReason` 的低置信度最终结果，并以 `SUCCEEDED` 结束。
