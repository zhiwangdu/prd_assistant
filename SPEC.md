...
- Tool Runner MVP 支持白名单工具配置、规则版多输入工具 action、`RUN_TOOL` phase、Claude MCP `logagent.run_domain_tool`、`tool_results` artifact 和 JSON stdout summary/findings 解析；真实 InfluxQL、Flux、openGemini storage 和 InfluxDB 1.x storage analyzers 已通过 `third_party/` submodules 引用，并由 `scripts/build-tools.sh` 构建到 LogAgent 工具目录，`scripts/smoke-source-built-analyzers.sh` 可聚合运行四个真实 analyzer smoke。源码构建阶段支持通过 `LOGAGENT_SUBMODULE_BASE_URL` 或各 `LOGAGENT_SUBMODULE_*_URL` 手动覆盖 submodule clone 地址，便于无法访问 GitHub 的内网镜像环境部署；该覆盖只写 submodule config，不得改写顶层仓库 `origin`。InfluxDB storage analyzer 构建必须让 `pkg-config.sh` 从本地 `third_party/flux` 完整源码树构建 `libflux`，避免 Go module cache 中缺失 Rust workspace 导致 cargo 失败；这段临时构建在未显式设置 `GOSUMDB` 时默认关闭公共 checksum DB 查询。`/api/v2/tools` catalog 同时返回 `tools` 和兼容 alias `toolPlugins`，source-built analyzer 自动发现除显式 env 外也会扫描当前工作目录和源码目录的标准 tools 路径。`sourceBuiltAnalyzers` 会报告命令文件是否存在、是否可执行和不可用原因；task MCP 和 Agent provider 只广告当前进程真实 runnable 的 configured subprocess tool。`flux_query_analyzer` stdout 可从 `metrics/topQueries/parseErrors` 生成 summary/findings；configured subprocess findings 会把当前输入 artifact 绝对路径规整为 workspace-relative 逻辑路径；`scripts/smoke-influxql-analyzer.sh` 已覆盖真实 InfluxQL Report 和 CompareReport 两条 CLI 路径。
- Fetch endpoint MVP 支持 `fetch` 配置段、DevTools bash cURL 导入预览、endpoint CRUD、endpoint `schemaVersion=2` 迁移、`refreshPolicy.mode=manual_only`、加密 credential set、allowlist 出网校验、WebUI 手动运行、任务 MCP `logagent.list_fetch_endpoints` / `logagent.fetch` 和 `tool_results/<action_id>/result.json#response` evidence ref。当前 token refresh policy 明确为手动更新 credential，不支持自动刷新。任务 MCP Fetch 使用稳定 `act_fetch_<digest>` action id，并返回 Rust/V1 顶层状态字段；Fetch result 使用 Rust/V1 `schemaVersion=3` tool result envelope，并附带 V2 response-body artifact id/path；只读 HTTP MCP 只展示工具目录 descriptor，不允许执行 `logagent.fetch`。cURL importer 已支持安全长参 `--url`、`--user-agent` / `-A` 和 `--referer` / `-e`，并保持多 URL 和受控 header 拒绝。
- Log Analyzer 支持节点日志包预处理：匹配 `<packageId>_<instanceId>_<nodeId>_<yyyy_MM_dd_HH_mm_ss_micros>_logs.tar.gz` 的包会按节点和时间展开到 `extracted/<nodeId>/<timestamp>/{tsdb,stream,agent}/`，archive 内允许顶层包装目录和 `./` 等目录项，路径中匹配 `var/chroot/gemini/log/{tsdb,stream}` 或 `home/Ruby/log` 的文件会归类到对应日志组；manifest 会记录 V1 样式 upload/package 摘要、log group 文件数、gzip 压缩统计、ignored path 样例和 file-level package metadata；目录内轮转日志不依赖文件名后缀，gzip 内容按 magic bytes 透明解码，并生成 `tool_inputs/index.json`、通用日志 JSONL 和 `influxql_analyzer` JSONL 输入。匹配当前工具的 materialized input 会作为独占自动输入，避免工具继续回退读取原始轮转日志。节点包若没有任何支持日志目录会失败并返回明确错误，避免空 manifest 被误判为成功解包。
- 普通文本上传和普通 archive 上传会使用稳定 `extracted/<uploadDir>/...` 逻辑路径；同名上传使用 `_2` 等后缀去重，保证 manifest、grep、log slice 和 Tool Runner fallback 在多上传场景中没有路径歧义。
...
- Analysis Orchestrator 已支持 Claude MCP `request_approval` 进入 `WAITING_FOR_APPROVAL`，通过 `POST /api/tasks/:task_id/actions/:action_id/decision` 批准或拒绝后恢复；审批后的 Remote Executor 命令、V2 SCP 文件采集和 approved `targets[]` 批量采集已接入 Environment Collector evidence 闭环。
- V2 LangGraph Agent runtime 已按 V1 默认值实现最大轮次、最大 LLM 调用、最大动作数、重复工具 fingerprint、总 token、单次运行时间、用户追问和审批预算终止；预算耗尽或重复动作触发会停止继续调用 provider / 跳过重复工具执行，生成带 `budgetLimited=true` 和 `terminationReason` 的低置信度最终结果，并以 `SUCCEEDED` 结束。
...
- 基于真实生产 Flux 查询日志继续扩展 `flux_query_analyzer` 输入转换深度和 baseline 新模板解释；基础 `metrics/topQueries/parseErrors` stdout 已可转成 summary/findings。
- 基于真实 TSSP/TSI/TSM/series fixture 扩展 openGemini 和 InfluxDB storage analyzers 的解析深度和 finding 规则。
- Fetch 后续如有需要再扩展 OAuth/API-token 自动刷新 provider；当前 endpoint schema v2 已固定 manual-only refresh policy，credential 更新通过 API/import 显式完成。
