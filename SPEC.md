...
- **dev_selftest**：在 Linux server 上提供 `sync_workspace`、`build`、`deploy`、`run_tests`、`report` 受控 MCP step tools，以及显式可选的 `cleanup` 环境清理 step 和只读 `diagnose` 诊断 step，由外部 MCP 客户端的本地 skill（如 Windows 上的 Claude Code）编排；源码只通过 allowlisted git repo/ref 同步，Windows 端 commit/push 后 ToolHub clone 或 pull，每次 run 有持久工作区 + run history。
...
| Dev Self-Test | git-only sync_workspace/build/deploy(docker)/run_tests/report MCP step tools + optional cleanup + read-only diagnose；完整 workflow 由客户端 skill 编排，docker runner 复用 remote_execution 的 docker 分支；`run_tests` 支持受限 `testParams` string map 注入 Docker 测试容器，供外部 skill 创建云实例后传入 instance/case/endpoint 等非敏感运行参数；git repo/ref allowlist 可通过受控热更新追加并设默认；Docker-backed build/test profile 可在用户确认后受控 upsert；默认 demo 覆盖 openGemini 集群和 InfluxDB OSS v1 单机；cleanup 只执行配置化 compose project 的 `docker compose down` 并保留 run 证据；diagnose 只读取 bounded evidence 并执行 allowlisted Docker 只读探测。 |
...
- InfluxDB dev_selftest 默认只构建 `master-1.x` 的单机 `influxd`，通过 Docker build profile 产出 Linux binary，并用单容器 compose + v1 HTTP smoke 验证。
