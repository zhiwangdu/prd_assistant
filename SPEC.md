...
- Agent Backends 已支持 `claude_agent_sdk`、`codex_cli`、`claude_code_cli` 和 `opencode_cli` 配置解析、默认后端校验、Settings 摘要和 dry-run 诊断；Log Analysis runtime 只调用 `claude_agent_sdk`，其它外部后端仍为预留配置类型。
