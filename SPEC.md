...
- V2 LangGraph Agent runtime 已按 V1 默认值实现最大轮次、最大 LLM 调用和最大动作数预算；预算耗尽会停止继续调用 provider，写入 `budget_guard` 审计响应，生成带 `budgetLimited=true` 和 `terminationReason` 的低置信度最终结果，并以 `SUCCEEDED` 结束。
