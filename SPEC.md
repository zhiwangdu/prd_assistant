...
- `scripts/v2-local.sh` 提供 V2 本地快速 build/start/stop/restart/status/logs/smoke-tools，默认复用 `server-v2/.venv`、`/tmp/logagent-v2-local`、端口 `50993` 和 `target/tools`，显式 `--with-tools` / `--only-tool` 时才构建 source-built analyzers；本地和 runtime `status` 会在服务可达时用 API Key 查询 `/api/v2/tools` 并打印 source-built analyzer 注册状态。`scripts/smoke-source-built-analyzers.sh` 提供全量或单工具 source-built analyzer smoke 入口，`v2-local.sh smoke-tools` 复用该聚合入口。
