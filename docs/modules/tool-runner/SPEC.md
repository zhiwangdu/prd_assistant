...
Server 还实现内置 Fetch runnable tool `logagent.fetch`。它复用 `tool_run`、Tools 目录和 `tool_results` artifact，但执行由 `fetch.enabled`、AES-256-GCM credential store、HTTP allowlist 和 reqwest 负责；不调用外部命令，也不导出到 `tools.zip`。
