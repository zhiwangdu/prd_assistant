...
V2 Server 已实现 Tool Runner MVP：配置白名单、规则版工具 action、Claude MCP `logagent.run_domain_tool`、可恢复 tool execution、timeout、stdout/stderr/result 持久化、stdout JSON 摘要解析和 artifacts API 展示。真实工具可通过 V2 environment variables、runtime `bin/tools` auto-discovery 或 `LOGAGENT_V2_TOOLS_JSON` 接入。当前四个工具源码通过 `third_party/` submodules 引用，并由 `scripts/build-tools.sh` 构建部署；构建阶段可通过 `LOGAGENT_SUBMODULE_BASE_URL` 或各 `LOGAGENT_SUBMODULE_*_URL` 覆盖 submodule clone 地址，适配内网镜像。
...
`LOGAGENT_TOOL_*_ANALYZER` 别名会生成 V2 built-in configured descriptors；
V2 专用变量优先生效。Flux/InfluxQL
...
- V2 `/api/v2/tools/runs/:run_id/result` 在 tool run 未成功前必须返回
  ...
  `resultPath`，方便迁移调用方不解析嵌套 run。手动 configured tool 如果选中
