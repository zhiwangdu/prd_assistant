...
- 可选 `tool_inputs/index.json`，由日志包预处理生成。Tool Runner 自动选择输入时优先使用声明给当前 toolId 的 materialized input；只要存在匹配项，就只使用这些 tool-ready 输入并受 `max_input_files` 限制。只有没有显式输入且没有匹配 materialized input 时，才回退到 manifest file pattern 和 grep keyword。
