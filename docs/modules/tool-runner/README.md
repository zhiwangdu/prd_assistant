...
`server-tools.yaml` 可配合 mock Claude CLI 单独验证 Tool Runner。上传 `.flux` 或包含 `flux/planner` 关键词的日志会触发 `flux_query_analyzer`；上传 `.jsonl` 或包含 `influxql`、`"query"`、`select`、`show series`、`show measurements` 关键词的日志会触发 `influxql_analyzer`。
