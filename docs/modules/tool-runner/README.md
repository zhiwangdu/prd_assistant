...
Tool Runner 负责调用已有编译好的诊断工具，并把工具输出转成 Agent Backend 可引用的结构化证据。Domain Adapter 负责说明某类系统适合哪些工具，Tool Runner 仍然只按 Server 白名单执行。

调用来源可以是初始规则或 Analysis Orchestrator 的 `run_tool` action，但都必须由 Server 映射到配置中的工具和参数模板。Agent Backend 不能提供可执行路径或自由 argv。

当前 Server 已实现共享 `AgentAction`、`EvidenceArtifact`、`EvidenceProvider` 契约和 `RUN_TOOL` phase。Tool Runner MVP 作为 Server 内部 Rust 模块运行，当前由 Server 规则根据 `manifest.json` / `grep_results.json` 自动生成 `run_tool` action；后续 Analysis Orchestrator 可复用同一个 action 执行通道。
