...
- 内置日志包预处理 tool 当前为 `logagent.preprocess_log_package`，支持批量 `.tar.gz` / `.tgz` 上传，运行结果写入 `tool_results/<action_id>/result.json`，摘要包含节点、日志组、轮转 gzip、忽略文件和 materialized tool inputs。
