...
调用来源可以是初始规则、Tools 页面手动运行或 Claude MCP `logagent.run_domain_tool`，但都必须由 Server 映射到配置中的工具和参数模板。Claude Code 不能提供可执行路径或自由 argv。Server 还提供内置 `logagent.preprocess_log_package` 和 `logagent.fetch` runnable 工具；前者复用 Analyze 解压链路并生成 tool-ready inputs，后者复用 `tool_run` / `tool_results` 展示和 evidence 机制但执行边界来自 `fetch` 配置、credential store 和 HTTP allowlist。
