...
- Agent Backend Adapter 调用 `internal_llm`、Claude Agent SDK、Codex CLI、Claude Code CLI 或 OpenCode CLI。
...
第一阶段新增 Agent Backend 配置/诊断、Domain Adapter registry 和外部 agent contract artifacts，但现有任务执行路径仍默认使用 `internal_llm`，Claude Agent SDK / CLI 后端尚不参与 `PLAN_ANALYSIS`。
...
每轮模型决策前，Orchestrator 会把当前证据刷新到 `analysis_package.json`，并写入 `agent_request.json` / `agent_response.json`。这三份文件用于后续 Claude Agent SDK adapter 接入；当前 `agent_response.json` 标记为 `not_invoked`，不改变现有执行路径。
