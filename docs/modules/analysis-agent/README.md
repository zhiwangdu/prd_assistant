...
已实现 Analysis State Store MVP，并启用 `PLAN_ANALYSIS` Claude Code session orchestration。用户追问和审批恢复 API 已启用；`collect_environment` 批准后可接入 Remote Executor 白名单命令、通过 V2 白名单 file template 拉取单个有大小上限的 SCP 文件，或通过审批输入中的 `targets[]` / `remoteTargets[]` 批量采集多个远程目标。多 executor / 多模板场景已支持基于 `target` / `executor` / `node` / `host` 和 `template` / `command` / `file` hint 的确定性唯一匹配；匹配不到或有歧义时拒绝执行远程采集。更多内置环境模板仍在后续。
...
`LOGAGENT_V2_AGENT_MAX_REPEATED_ACTION_FINGERPRINTS=1`。当轮次、模型调用、
动作预算或重复 task MCP tool fingerprint 耗尽时，V2 会停止继续调用
