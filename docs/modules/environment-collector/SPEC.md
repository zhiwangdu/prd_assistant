...
Agent 可见候选摘要和单 executor 自动补齐；多 executor 语义自动选择和更丰富环境模板尚未实现：
...
- V2 已接入 `collect_environment` 审批后的 evidence 闭环：批准 input 带
  ...
  `remoteInput` 中。批准 input 也可携带最多 20 个 `targets[]` /
  `remoteTargets[]` 批量目标；每个目标必须指向已启用 executor，或可通过
  唯一 executor 规则补齐，并选择一个白名单 command/file 模板。单目标完成后写入
...
- V2 `analysis_package.environmentCollection` 必须暴露已启用 executor、
  command/file 模板和单 executor 推断规则，供 Agent 构造结构化
  `collect_environment` 审批请求。当前批量采集需要审批输入显式提供
  `targets[]`；多 executor 语义自动选择和更多内置环境模板仍未实现。
...
- V2 批量采集要求批准 input 包含 `targets[]` / `remoteTargets[]`，数组长度为
  ...
  使用唯一 executor 推断，并且只选择已配置的 `commandId` 或 `fileId` 之一
...
- V2 `analysis_package.environmentCollection` 必须列出当前可用于
  ...
  `executorSelection` 规则，且不包含已禁用 executor。
- V2 API 批准 `collect_environment` 时如果 decision body 提供
