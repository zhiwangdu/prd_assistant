...
- V2 已接入 `collect_environment` 审批后的 V1-compatible mock 证据：批准后写入 `environment_evidence/<action_id>/result.json`，状态为 `MOCK`，作为下一轮 Agent 的 background evidence。
- 当前未把 `collect_environment` 映射到真实 SSH/SCP Environment Collector，也未实现 SCP 文件拉取和多节点采集。
