...
已实现通用 Remote Executor 基础框架和 V2 审批后的单目标/结构化批量环境采集；Agent 自动选择目标和更丰富环境模板尚未实现：
...
- V2 已接入 `collect_environment` 审批后的 evidence 闭环：批准 input 带
  ...
  SCP 拉取单个有大小上限的远程文件。批准 input 也可携带最多 20 个
  `targets[]` / `remoteTargets[]` 批量目标；每个目标必须指向已启用 executor
  和一个白名单 command/file 模板。单目标完成后写入
...
- 当前批量采集需要审批输入显式提供 `targets[]`；仍未实现 Agent 自动选择
  executor/command/file 和更多内置环境模板。
...
- V2 批量采集要求批准 input 包含 `targets[]` / `remoteTargets[]`，数组长度为
  1..20；每个 target 均包含已存在的 `executorId`，并且只选择已配置的
  `commandId` 或 `fileId` 之一
