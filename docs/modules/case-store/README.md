...
只读 HTTP MCP 通过 `logagent://cases/recent`、`logagent.search_cases` 和 `logagent.get_case` 暴露已确认 Case，供个人本地 Claude Code 读取；该入口不能创建、编辑、禁用或确认 Case。`logagent://cases/recent` 返回 Rust/V1 默认的最近 20 个 enabled Case；`logagent.search_cases` 保持 Rust/V1 只读入口的默认 `limit=20` 和 `limit=1..50` 上限。
