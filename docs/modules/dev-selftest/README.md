...
- **P2 docker 切片（已实现）**：把 executor 执行逻辑抽成可复用的 `run_executor_command`
  ...
  端口访问集群 ts-sql。`DevSelftestTestDocker`（image/network/workdir/volumes/env）在 `enabled`
  时做安全校验（见 `server/SPEC.md`）。默认 demo：`deploy/devselftest/opengemini/tests/smoke.sh`
  （alpine + busybox wget，CREATE/INSERT/SELECT，无 apt/外网依赖）。fake-docker 单测 +
  集成 `docker_executor_test_dispatch` + SSH fake 回归全绿。**不在本切片**：Docker executor
  record / `/api/executors` docker CRUD / run history 纳管、参数化命令模板（`{var}`+Schema）、
  `ssh_binary_replace`。
- **P2 仍 deferred**：参数化 executor 命令模板（`{var}` + 小 JSON Schema）、Docker executor
  纳管（record + CRUD + run history）、受控 file-template SCP + `ssh_binary_replace` 部署
  （备份/重启/health/回滚）。
