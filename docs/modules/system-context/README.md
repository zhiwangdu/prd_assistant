...
- LLM Gateway 将 `system_context.json` 作为背景参考注入 Prompt。
...
System Context 只作为背景参考进入 Prompt。固定安全约束、结构化输出 schema 和 allowed actions 仍由代码追加，Prompt Pack 不能覆盖 Server 侧校验。
