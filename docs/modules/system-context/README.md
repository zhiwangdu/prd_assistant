...
- `GET /api/skills`、`GET /api/skills/:skill_id`、`POST /api/skills/imports` 和 `POST /api/skills/preview` 负责新的 Skill 入口；导入写入第一个配置的 Skill root，成功后可被后续 task 快照选择。
- MCP `resources/read system_context` 继续返回 `system_context.json`。
- 只读 HTTP MCP 提供 `logagent.preview_system_context`，可按 Skill IDs、产品/版本/环境或 Metadata instance 预览将注入的 Skill-backed System Context；返回合并 `resources`、拆分 `skillResources` / `systemResources` 和 prompt preview，但不创建 task、不写 `system_context.json`。
- V2 兼容实现提供 `/api/v2/system-context/resources` 和
  `/api/v2/system-context/preview`，用 SQLite 管理 V1 风格资源、版本和
  prompt 预览。V2 的新分析运行仍走 Skill-backed System Context，不自动注入这些
  兼容资源。
- V2 Metadata instances 会在兼容资源列表中以只读 `meta_<instanceId>`
  / `metadata_instance` adapter 形式出现，可显式加入 preview。
- WebUI 的 V2 System Context Workbench 已接入兼容资源管理入口，可创建
  V1 风格资源、编辑摘要字段、追加/编辑/激活版本，并预览显式选择的资源或
  Metadata adapter；这些资源仍不作为新分析运行的默认知识主路径。
...
- 把兼容 Prompt Pack、Architecture、Runbook、Glossary、Tool Capability 或 Knowledge Note 作为新运行时知识的主形态；长期知识优先沉淀为 Skills。
...
data_dir/
  ...
  logagent.sqlite    # V2 system_context_resources table
  workspaces/
    task_xxx/
      system_context.json
      skill_references/
