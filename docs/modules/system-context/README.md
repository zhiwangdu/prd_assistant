...
- Claude Code 会通过 `analysis_package.json` 和 MCP resource 接收 `system_context.json` 摘要作为背景参考。
...
System Context 只作为背景参考进入 Claude Code 输入。固定安全约束、结构化输出 schema 和 MCP tool 白名单仍由代码追加，Prompt Pack 和 Domain Adapter 内容不能覆盖 Server 侧校验。
