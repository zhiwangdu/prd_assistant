...
本模块替代旧的 Claude Code Session Runner 叙事。LocalToolHub 不再默认把 Claude Code 当作 Server 后端；Claude Code、Codex、Cursor、OpenCode 等都只是外部 MCP client。

## 目标

- 提供 MCP endpoint、tools/resources 和配置示例。
- 说明外部 Agent 如何连接 LocalToolHub。
