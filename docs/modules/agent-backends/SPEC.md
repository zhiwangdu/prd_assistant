...
- Log Analysis run 在每轮 `PLAN_ANALYSIS` 前写出 `analysis_package.json` 和 `agent_request.json`，调用 `claude_agent_sdk` adapter 后写出真实 `agent_response.json`。
...
`analysis_package.json` 包含任务、用户问题、manifest、grep、Metadata、System Context、Case、Tool capability、Tool results 和 analysis state 摘要。`agent_request.json` 声明默认后端、输入包、允许输出 schema 和 Server 执行策略。`agent_response.json` 记录真实 adapter 响应，包含 `runtimeStatus`、原始 decision、`normalizedDecision`、usage/cost、`durationMs` 和错误信息。

`claude_agent_sdk` adapter 调用协议：

```text
<command_path> run --request agent_request.json --package analysis_package.json
```

adapter 的 stdout 必须是纯 JSON，可直接是 `AgentDecision`，也可使用包含 `decision` 或 `normalizedDecision` 的 envelope。未知 action、自由命令、任意路径、任意 SSH 目标和非法 evidence ref 必须拒绝。
...
- adapter 非零退出、超时、stdout 非 JSON、非法 action 或非法 evidence ref 会写入失败 `agent_response.json` 并使任务失败，不自动 fallback。
