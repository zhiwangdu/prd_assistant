...
- Python V2 未设置 `LOGAGENT_V2_REMOTE_COMMANDS_JSON` 时默认暴露 `smoke_ls_root`、`system_uname`、`uptime_load`、`disk_usage`、`memory_usage`、`process_overview` 和 `network_listeners` 只读命令模板；设置该环境变量会替换整套默认模板。
