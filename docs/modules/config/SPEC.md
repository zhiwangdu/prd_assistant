...
- Python V2 `LOGAGENT_V2_GREP_KEYWORDS` 是逗号分隔的初始 grep 关键词，默认与 Rust/V1 `log_analyzer.keywords` 对齐为 `error,exception,timeout,fail,failed,panic,fatal,refused,denied,verify`；`LOGAGENT_V2_MAX_GREP_MATCHES` 控制初始命中上限，默认与 Rust/V1 `log_analyzer.max_matches` 对齐为 `200`。
