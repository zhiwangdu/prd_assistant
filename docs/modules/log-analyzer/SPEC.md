...
- Claude MCP `logagent.search_logs` 后续检索写入独立 `log_searches/logsearch_*.json`，返回 `matches[].text`、`keywordCounts`、`unmatchedKeywords` 和稳定 `log_searches/...#matches/<index>` evidence refs，不覆盖初始 `grep_results.json`；MCP 响应同时提供 Rust/V1 兼容的顶层 `artifactPath`、`totalMatches`、`matches[].index`、`matches` 和 `evidenceRefs`
