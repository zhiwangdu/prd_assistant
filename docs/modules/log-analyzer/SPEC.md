...
Python V2 对应环境变量为 `LOGAGENT_V2_GREP_KEYWORDS` 和 `LOGAGENT_V2_MAX_GREP_MATCHES`；初始关键词默认与 Rust/V1 对齐为 `error,exception,timeout,fail,failed,panic,fatal,refused,denied,verify`，不会自动从用户问题拆词。
