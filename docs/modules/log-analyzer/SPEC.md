...
- 匹配 `<packageId>_<instanceId>_<nodeId>_<timestamp>_logs.tar.gz` 的节点日志包按 `extracted/<nodeId>/<timestamp>/{tsdb,stream,agent}/` 展开
...
无上传的 Session task 输入为空，但仍提供 Task workspace；Log Analyzer 对空 `extracted_dir` 返回空清单和空 grep 结果。节点日志包预处理只保留 `/var/chroot/gemini/log/tsdb`、`/var/chroot/gemini/log/stream` 和 `/home/Ruby/log` 三类目录，其他普通文件忽略并在 manifest upload summary 中记录数量和样例路径。
...
- 节点日志包按 nodeId/timestamp/logGroup 展开，轮转 gzip 文件可被 grep 和日志切片读取。
