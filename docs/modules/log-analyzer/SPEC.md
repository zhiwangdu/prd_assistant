...
- 匹配 `<packageId>_<instanceId>_<nodeId>_<timestamp>_logs.tar.gz` 的节点日志包按 `extracted/<nodeId>/<timestamp>/{tsdb,stream,agent}/` 展开；archive 内允许顶层包装目录，路径中出现 `var/chroot/gemini/log/{tsdb,stream}` 或 `home/Ruby/log` 即可分类
...
- 接收经 Server 校验的 Claude MCP `logagent.search_logs` 请求。
...
无上传的 Session task 输入为空，但仍提供 Task workspace；Log Analyzer 对空 `extracted_dir` 返回空清单和空 grep 结果。节点日志包预处理只保留路径中匹配 `/var/chroot/gemini/log/tsdb`、`/var/chroot/gemini/log/stream` 和 `/home/Ruby/log` 的三类目录，archive 顶层可额外包一层或多层目录；其他普通文件忽略并在 manifest upload summary 中记录数量和样例路径。
...
- 节点日志包按 nodeId/timestamp/logGroup 展开，archive 顶层包装目录不会阻止识别日志路径，轮转 gzip 文件可被 grep 和日志切片读取。
- 节点日志包没有任何支持日志目录时任务失败并给出明确错误，不产生空成功 manifest。
- `tool_inputs/index.json` 中的 `influxql_analyzer` 输入只包含明确提取出查询文本的记录。
- 路径逃逸压缩包被拒绝。
- manifest 路径使用 `/` 分隔。
