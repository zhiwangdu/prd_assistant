...
- If a real model returns a bare final-result JSON during `PLAN_ANALYSIS` without the outer `type` field, LLM Gateway wraps it as `final_answer` and still validates evidence refs.
